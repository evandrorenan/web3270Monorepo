export const NEW_SESSION = "NEW_SESSION";
export const GET_SCREEN = "GET_SCREEN";
export const GET_SCREEN_FIELDS = "GET_SCREEN_FIELDS";
export const SET_STATUS = "SET_STATUS";
export const CREATE_REF = "CREATE_REF";
export const SEND_KEYS = "SEND_KEYS";
export const SET_FIELD_TEXT = "SET_FIELD_TEXT";
export const UPDATE_POSITION_TEXT = "UPDATE_POSITION_TEXT";
export const MARK_MODIFIED_FIELD = "MARK_MODIFIED_FIELD";
export const SET_FOCUSED_FIELD = "SET_FOCUSED_FIELD";
export const SET_FOCUS = "SET_FOCUS";
export const DUMMY = "DUMMY";
export const SET_REPORT_OBJECT = "SET_REPORT_OBJECT";
export const SET_STOMP_CLIENT = "SET_STOMP_CLIENT";
export const SET_DOWNLOAD_OBJECT = "SET_DOWNLOAD_OBJECT";
export const SET_WAITING_STATUS_TRUE = "SET_WAITING_STATUS_TRUE";
export const SET_WAITING_STATUS_FALSE = "SET_WAITING_STATUS_FALSE";
