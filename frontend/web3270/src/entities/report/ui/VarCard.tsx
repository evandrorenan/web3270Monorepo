...
export const VarCard: React.FC<VarCardProps> = ({ varName, pinned, onPin, onExpand }) => {
  ...
  return (
    <div className={`rounded-lg border p-2.5 font-mono transition-all
      ${varData.corrupt
    ...
    `}>
      ...
      <div className="flex items-center gap-1 mb-1">
        ...
        <button
          ...
          className={`text-[9px] px-1 py-0.5 rounded border leading-none shrink-0 transition-colors
            ${hex
        ...
        {isTruncated && (
          <button
            onClick={() => onExpand({ varName, varData })}
            title="Expand full value"
            className="text-[9px] px-1 py-0.5 rounded border border-slate-700 text-slate-500 hover:border-slate-400 hover:text-slate-200 leading-none shrink-0"
          >⤡</button>
        )}

        {/* Pin */}
        <button
          onClick={() => onPin(varName)}
          title={pinned ? "Unpin" : "Pin — keep always visible"}
          className={`text-[11px] px-1 py-0.5 rounded border leading-none shrink-0 transition-colors
            ${pinned
      ...
      {/* Meta */}
      <div className="text-[9px] text-slate-600 mb-1 leading-none">
      ...
      <div
        ...
        onClick={() => isTruncated && onExpand({ varName, varData })}
      >
