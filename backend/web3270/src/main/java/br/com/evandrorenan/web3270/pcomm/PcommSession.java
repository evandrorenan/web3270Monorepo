...
public class PcommSession extends ECLSession implements IMySession {
	...
	private static final Logger logger = LoggerFactory.getLogger(PcommSession.class);

	private String sessionId;
	
	public PcommSession(SimpMessagingTemplate messageTemplate) throws ECLErr {
		...
		if (this.GetCommStatus() != ECLConnection.CONNECTION_READY
		&&  this.GetCommStatus() != ECLConnection.CONNECTION_ACTIVE) {
			this.sessionId = null;
			return;
		}
		...
		addEventListener(messageTemplate);		
	}

	private void addEventListener(SimpMessagingTemplate messageTemplate) {
		this.GetPS().RegisterPSEvent(new PcommEventListener(this, messageTemplate ));
		this.RegisterCommEvent(new PcommCommunicationEventListener(), true);
	}
	
	private static Properties tn3270Properties() {
		Properties props = new Properties();
		
		props.setProperty("SESSION_HOST", 		"192.168.240.1");
		props.setProperty("SESSION_HOST_PORT", 	"51004");
